...
function FormBody({ slot, activityId, activityTitle, onClose }: { slot: Slot; activityId: number; activityTitle: string; onClose: () => void }) {
  ...
  return (
    <>
      <div className="flex items-center justify-between border-b border-border px-6 py-4">
        ...
        <button onClick={onClose} className="ml-4 rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-muted hover:text-navy shrink-0">
          <X className="h-5 w-5" />
        </button>
      </div>
    ...
    <>
      ...
      {isMobile && (
        <Drawer open={open} onOpenChange={setOpen}>
          <DrawerContent className="max-h-[90vh] px-0 pb-0">
            <FormBody slot={slot} activityId={activityId} activityTitle={activityTitle} onClose={() => setOpen(false)} />
          </DrawerContent>
        </Drawer>
      )}
      ...
      {open && (
        <div className="hidden sm:flex">
          ...
          >
            <div className={`flex w-[80vw] h-[80vh] flex-col rounded-2xl bg-white shadow-2xl transition-all duration-200 ${visible ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"}`}>
              <FormBody slot={slot} activityId={activityId} activityTitle={activityTitle} onClose={close} />
            </div>
          </div>
        </div>
      )}
