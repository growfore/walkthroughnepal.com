  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/inquiry", {
        ...
        body: JSON.stringify({ fullName, email, phone, destination, groupSize, message }),
      })
            ...
            <div className="rounded-2xl border border-border bg-card p-6 sm:p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  ...
                  <div>
                    <label className="mb-1.5 block text-sm font-semibold text-navy">Destination</label>
                    <input value={destination} onChange={(e) => setDestination(e.target.value)} className="w-full rounded-lg border border-border bg-background px-4 py-2.5 text-sm outline-none focus:border-orange focus:ring-2 focus:ring-orange/20" placeholder="e.g. Everest Base Camp" />
                  </div>
                </div>
                <div className="grid gap-5 sm:grid-cols-2">
                  <div>
